...
set :public_folder, TORRENTSYNC_HOME + 'public'
...
post '/upload' do
  ...
  if /\.torrent$/ === origname
    body = File.open(path, 'rb'){|fd| fd.read}
    dest = File.join(torrents, origname)
    FileUtils.mv(path, dest)
    FileUtils.chmod(0644, dest)
    torrent = BEncode.load(body)
  else
